...
// ═══════════════════════════════════════════════════════════════
function calcMetaAbsenteismo() {
    var hoje   = new Date(); hoje.setHours(0,0,0,0);
    var ano    = hoje.getFullYear();
    var mes    = hoje.getMonth();
    var anoMes = ano + '-' + String(mes+1).padStart(2,'0');

    // Quadro ativo real (apenas status ATIVO)
    var listaRH = window.dRH || [];
    var rh      = window.rhData || {};
    ...
    // Dias úteis decorridos e total do mês (seg-sex)
    var diasUteisAte  = 0;
    var diasUteisMes  = 0;
    ...
    var taxa = diasUteisAte > 0
        ? (diasAusentes / (quadroAtivo * diasUteisAte)) * 100 : 0;
    var maxDias   = 0.04 * quadroAtivo * diasUteisMes;
    var saldo     = Math.floor(maxDias - diasAusentes);
    var barWidth  = Math.min((taxa / 8) * 100, 100);
    var barColor  = taxa <= 2 ? '#27ae60' : taxa <= 4 ? '#f39c12' : '#e74c3c';
    var atingiu   = taxa <= 4;
    ...
    setSt('abs-gauge-fill','width', barWidth+'%');
    setSt('abs-gauge-fill','background', barColor);
    set('abs-gauge-pct', taxa.toFixed(2)+'%');
    setSt('abs-gauge-pct','color', barColor);
    ...
    if (listEl) {
        listEl.innerHTML = arr.length ? arr.slice(0,15).map(function(x){
            ...
            return '<div class="meta-impact-item">' +
                ...
                '</span></div>';
        }).join('') : '<div style="padding:16px;text-align:center;color:#999;font-size:12px;">Nenhuma ausência registada no mês.</div>';
    }

    // IA
    var ia = '';
    if (taxa === 0) {
        ia = '✅ <strong>Perfeito!</strong> Nenhuma ausência registada no mês. Manter monitoramento diário.';
    } else if (taxa <= 2) {
        ia = '✅ Índice <strong>'+taxa.toFixed(2)+'%</strong> — excelente. Saldo de <strong>'+saldo+' dias</strong> disponíveis. Continue o acompanhamento.';
    } else if (taxa <= 4) {
        ia = '⚡ Índice <strong>'+taxa.toFixed(2)+'%</strong> — atenção. Saldo restante: <strong>'+saldo+' dias</strong>.<br>'+
             '• Conversar com colaboradores de maior impacto<br>• Monitorar presença nos próximos dias<br>• Evitar aprovações de folga neste período';
    } else {
        var rest = diasUteisMes - diasUteisAte;
        var podeRecuperar = (maxDias - diasAusentes) > 0;
        ia = '🚨 <strong>META EM RISCO!</strong> Índice: <strong>'+taxa.toFixed(2)+'%</strong> (meta: 4%).<br>'+
             '• Acionar gestores dos colaboradores listados abaixo<br>'+
             '• Solicitar atestados e documentação das ausências<br>'+
             '• Não aprovar novas folgas até fim do mês';
        if (rest > 0 && podeRecuperar) {
            var taxaMax = ((maxDias-diasAusentes)/(quadroAtivo*rest)*100).toFixed(2);
            ia += '<br>• Nos <strong>'+rest+' dias úteis restantes</strong>, taxa máxima permitida: <strong>'+taxaMax+'%</strong>';
        } else if (!podeRecuperar) {
            ia += '<br>⚠️ Meta mensal <strong>não pode mais ser recuperada</strong>. Foque no próximo mês.';
        }
    }
    setH('abs-ia-text', ia);
...
function processarRssPorDia(rows) {
    // Agrupa por data: cada linha pode ser um dia ou um produto num dia
    ...
    function col(r) {
        ...
        return {
            data:     find('data','date','dia','day'),
            pedido:   parseFloat(find('pedido','ped','qtdped','solicitado','req','total_ped','quantidade_pedida')) || 0,
            atendido: parseFloat(find('atendido','ate','qtdate','separado','att','total_ate','quantidade_atendida')) || 0
        };
    }
    // Agrupa por data
    var mapaData = {};
    rows.forEach(function(r) {
        var c = col(r);
        if (!c.data || c.pedido === 0) return;
        // Normaliza data
        var dt = c.data;
        if (dt.includes('/')) { var p=dt.split('/'); dt=p[2]+'-'+p[1].padStart(2,'0')+'-'+p[0].padStart(2,'0'); }
        if (!mapaData[dt]) mapaData[dt] = { data:dt, pedido:0, atendido:0 };
        mapaData[dt].pedido   += c.pedido;
        mapaData[dt].atendido += c.atendido;
    });
    return Object.values(mapaData).sort(function(a,b){ return a.data.localeCompare(b.data); });
}
...
// =====================================================================
// ATUALIZAÇÃO DEFINITIVA: DASHBOARD INTELIGENTE NATIVO + EXCLUIR
// =====================================================================

// 1. FUNÇÃO NATIVA DE EXCLUSÃO (Firebase)
window.excluirColaboradorRH = async function(idColab) {
    if (!confirm("⚠️ Atenção: Deseja realmente EXCLUIR este colaborador do sistema? Esta ação apagará os dados na nuvem.")) return;
    ...
    if (typeof window.saveToDB === 'function') {
        window.saveToDB('rh', novaLista);
    } else if (typeof window.saveDataSafe === 'function') {
        window.saveDataSafe('rh', novaLista);
    }

    alert("🗑️ Colaborador excluído com sucesso!");
    
    // Atualiza a tabela chamando o renderRHQuad
    if (typeof window.renderRHQuad === 'function') window.renderRHQuad();
    else if (typeof window.renderRH === 'function') window.renderRH();
};

// 2. INJEÇÃO SEGURA NO renderRHQuad (Coloca o botão lado a lado)
if (typeof window.renderRHQuad !== 'undefined') {
    const originalRenderRHQuad = window.renderRHQuad;
    window.renderRHQuad = function() {
        // Executa a sua tabela original primeiro
        originalRenderRHQuad.apply(this, arguments);
        
        // Em seguida, adiciona nativamente o botão Excluir
        let telaRH = document.getElementById('rh-view') || document;
        let botoesModificar = telaRH.querySelectorAll('button');
        
        botoesModificar.forEach(btn => {
            let txt = btn.innerText.trim().toLowerCase();
            if ((txt === 'editar' || txt === 'modificar') && !btn.classList.contains('com-excluir-nativo')) {
                btn.classList.add('com-excluir-nativo');
...
// 3. MOTOR DE ANÁLISE RH (Gera insights usando apenas dados locais)
window.gerarInsightIAReal = async function() {
    let telaRH = document.getElementById('rh-view') || document.querySelector('.rh-dashboard-container');
    if (!telaRH) return;
    ...
    // MOTOR DE INFERÊNCIA (Cria o relatório automaticamente)
    let insights = "";
    let turnover = total > 0 ? ((inativos / total) * 100).toFixed(1) : 0;

    if (episAtrasados > 0) {
        insights += `<p style="margin-bottom:12px;"><span style="color:#e74c3c;"><i class="fas fa-exclamation-triangle"></i> <b>Alerta Crítico de EPI:</b></span> O sistema Firebase detectou <b>${episAtrasados}</b> entregas pendentes. <br><i>Ação sugerida:</i> Notificar a logística imediatamente para mitigar risco de passivo trabalhista e multas.</p>`;
    } else {
        insights += `<p style="margin-bottom:12px;"><span style="color:#2ecc71;"><i class="fas fa-shield-alt"></i> <b>Segurança e Compliance:</b></span> 100% dos registos de EPI estão em dia. A empresa não apresenta riscos legais neste quesito atualmente.</p>`;
    }

    if (turnover > 15) {
        insights += `<p style="margin-bottom:12px;"><span style="color:#f39c12;"><i class="fas fa-users-slash"></i> <b>Atenção à Retenção:</b></span> A taxa de inatividade atingiu <b>${turnover}%</b> (acima do limite saudável de 15%). <br><i>Ação sugerida:</i> Aplicar pesquisa de clima organizacional para identificar a raiz dos desligamentos.</p>`;
    } else {
        insights += `<p style="margin-bottom:12px;"><span style="color:#3498db;"><i class="fas fa-users"></i> <b>Equipa Estável:</b></span> O quadro de colaboradores está saudável, com taxa de retenção positiva (turnover de ${turnover}%).</p>`;
    }

    // Renderiza o Dashboard com a Análise Nativa
    divIA.innerHTML = `
        <div style="background: linear-gradient(135deg, #2c3e50, #34495e); color: white; padding: 20px; border-radius: 12px; margin-bottom: 25px; box-shadow: 0 4px 15px rgba(0,0,0,0.3); border-left: 5px solid #3498db;">
            <h3 style="margin-top:0; border-bottom: 1px solid rgba(255,255,255,0.2); padding-bottom: 10px; font-size:18px;">
                <i class="fas fa-microchip" style="color:#3498db;"></i> Motor de Análise RH
            </h3>
            <div style="display:flex; gap:15px; margin-bottom:15px; flex-wrap:wrap;">
                <div style="background:rgba(255,255,255,0.1); padding:10px; border-radius:8px; flex:1; text-align:center; min-width:100px;">
                    <h2 style="margin:0; color:#2ecc71;">${ativos}</h2><small>Ativos</small>
                </div>
                <div style="background:rgba(255,255,255,0.1); padding:10px; border-radius:8px; flex:1; text-align:center; min-width:100px;">
                    <h2 style="margin:0; color:#e74c3c;">${inativos}</h2><small>Inativos</small>
                </div>
                <div style="background:rgba(255,255,255,0.1); padding:10px; border-radius:8px; flex:1; text-align:center; min-width:100px;">
                    <h2 style="margin:0; color:#f1c40f;">${episAtrasados}</h2><small>EPIs Pendentes</small>
                </div>
            </div>
            <div style="background:white; color:#333; padding:15px; border-radius:8px; font-size:14px; line-height:1.6;">
                ${insights}
            </div>
            <div style="text-align: right; margin-top: 15px;">
                <button onclick="window.gerarInsightIAReal()" style="background:#2980b9; color:white; border:none; padding:8px 15px; border-radius:5px; cursor:pointer; font-weight:bold;">
...
// Solicita a análise assim que a tela abre, e atualiza sempre que clicar no botão
setTimeout(window.gerarInsightIAReal, 1000);
// =================================================================
// PACOTE SUPREMO FINAL: EXCLUIR + IA NATIVA + AMORTECEDOR FIREBASE
// =================================================================
...
window.excluirColaboradorRH = async function(idColab) {
    ...
    let bRH = window.dRH || window.dadosRH || JSON.parse(localStorage.getItem('sysLog_mem_rh') || '[]');
    let novaLista = bRH.filter(c => String(c.matricula) !== String(idColab) && String(c.id) !== String(idColab));
    window.dRH = novaLista; window.dadosRH = novaLista;
    localStorage.setItem('sysLog_mem_rh', JSON.stringify(novaLista));
...
// 2. INJEÇÃO DO BOTÃO EXCLUIR (PROCURA NA TELA TODA)
...
// 3. DASHBOARD RH INTELIGENTE (NATIVO)
window.gerarInsightIAReal = function() {
    ...
    if (episAtrasados > 0) insights += `<p style="margin-bottom:12px;"><span style="color:#e74c3c;"><i class="fas fa-exclamation-triangle"></i> <b>Alerta Crítico EPI:</b></span> ${episAtrasados} pendentes.</p>`;
